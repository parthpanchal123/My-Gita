...
export default async function Verse(context) {
  ...
  const [_, verseDetailData, commentaryDesc] = await Promise.all([
    verseId,
    verseData,
    commentaryData,
  ]);
  ...
  } = verseDetailData.data.allGitaVerses.nodes[0];
      ...
      <div className="min-h-screen bg-neutral-900 text-white">
        <div className="container h-full mx-auto pt-5 max-w-5xl p-2">
          <div className="flex flex-col gap-y-5 justify-center items-center text-center">
            ...
            <p
              className={`text-3xl text-orange-400 ${kalam.variable} font-display max-w-md`}
            >
              {text}
            </p>

            <p
              className={`text-xl text-white ${inter.variable} font-sans mx-auto max-w-md`}
            >
              {transliteration}
            </p>
                ...
                <>
                  <h1 className="text-md mb-2">
                    ...
                    <span className="text-orange-400 italic text-base">
                      {commentaryDesc.english.authorName}
                    </span>
                  </h1>
                  <p
                    className={`text-md text-justify ${inter.variable} font-sans`}
                  >
                    {commentaryDesc.english.description}
                  </p>
                </>
              }
            </div>
            ...
            {verseId !== "1" && (
              <Link href={`/verse/${parseInt(verseId) - 1}`}>
